...
console.log(result.vault?.reveal("<RRN_1>"));                  // 880101-1234568 (only authorized users can restore)
console.log(RiskLevel[result.combined_risk!.combined_risk]);   // CRITICAL
```

Python (the canonical implementation, 3.10+) produces the same result for the same input — TypeScript checks detection spans, pseudonymized output, and even Vault bytes against gold-master vectors generated by Python ([Multi-language implementation](#multi-language-implementation-python--typescript)).
...
const anon = new Anonymizer(
  ...
  undefined,                     // vault
  undefined,                     // include
  ["AGE", "HEIGHT", "WEIGHT"],   // exclude — avoid false positives like "per 1 kg of body weight"
);

// If there are many PERSON FPs — inject a domain dictionary
// Add pharma ingredient names / manufacturer names to src/ts/src/dictionaries/common_words.ts
...
const anon = new Anonymizer(ProcessingMode.AUDIT);  // no blocking, detection-only reporting
...
### Scenario 3 — Automatic PII pseudonymization in application logs (for developers)

```ts
// Wrap your logging call to pseudonymize just before output
import { Anonymizer, ProcessingMode } from "ko-pii";
...
console.log(report.k);                    // minimum group size
...
applyFeedback(
  "review.jsonl",
  "feedback_patches/",
  2,   // same token marked FP 2+ times → candidate (prevents dictionary pollution)
);
...
// Filter to a single label with `include` (TS has no patterns subpath; use detectAll)
...
> **Language support:** the LlamaIndex/LangChain adapters are Python-only. In TypeScript, apply `Anonymizer` directly to retrieval results to build the same pipeline.
...
> **Language support:** the HF token-NER adapter and document classifier below are torch-dependent and Python-only. In TypeScript, inject your own detector via the `SecondaryDetector` interface (see the [Multi-language implementation](#multi-language-implementation-python--typescript) table below).
