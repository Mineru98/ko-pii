...
const LABEL = "PNU";
const LEGAL_BASIS = "공간정보의 구축 및 관리 등에 관한 법률; 개인정보보호법 제2조";
const CATEGORY = "참조정보";

// 19자리 숫자, 주변에 다른 숫자가 없는 boundary
const PATTERN = /(?<![0-9])([0-9]{19})(?![0-9])/g;

// 시도 행정구역 코드 (행정안전부 표준)
// 11=서울 21=부산 22=대구 23=인천 24=광주 25=대전 26=울산 29=세종
// 31=경기 32=강원 33=충북 34=충남 35=전북 36=전남 37=경북 38=경남 39=제주
const VALID_SIDO_CODES: ReadonlySet<string> = new Set([
  "11",
  "21",
  "22",
  "23",
...
function parseValidPnu(digits: string): PnuInfo | null {
  if (digits.length !== 19) {
    return null;
  }
  const sido = digits.slice(0, 2);
  if (!VALID_SIDO_CODES.has(sido)) {
    return null;
  }
  const sigungu = digits.slice(2, 5);
  const eupmyeondong = digits.slice(5, 8);
  const ri = digits.slice(8, 10);
  const parcelType = digits.slice(10, 11);
  const bonbun = digits.slice(11, 15);
  const bubun = digits.slice(15, 19);
...
export function detect(text: string): DetectionResult[] {
  ...
  for (const m of text.matchAll(PATTERN)) {
    ...
    const info = parseValidPnu(digits);
    if (info === null) {
      continue;
    }
    results.push(
      makeDetection({
        ...
        confidence: 0.9,
        evidence: ["pattern:pnu", `sido:${info.sido_code}`, `parcel_type:${info.parcel_type}`],
